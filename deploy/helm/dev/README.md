...
2. Install the chart from the local directory:
   ...
   | `hub.image.tag=dev`, `hub.imagePullPolicy=Never` | Use the image loaded in step 1 instead of pulling from the registry. To test a published image instead, set the tag to a release (e.g. `2.1.1`) and drop the pull policy; the committed `Chart.yaml` only carries a baseline `appVersion` that the release workflow overrides |
