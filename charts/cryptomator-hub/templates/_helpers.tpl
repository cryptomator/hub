...
{{- define "cryptomator-hub.resolvedHubSyncerPassword" -}}
{{- if hasKey .Values "_resolvedHubSyncerPassword" -}}
{{- index .Values "_resolvedHubSyncerPassword" -}}
{{- else if .Values.hub.syncer.password -}}
{{- $_ := set .Values "_resolvedHubSyncerPassword" .Values.hub.syncer.password -}}
{{- index .Values "_resolvedHubSyncerPassword" -}}
{{- else -}}
{{- $secretName := print (include "cryptomator-hub.fullname" .) "-secrets-hub" -}}
{{- $existing := lookup "v1" "Secret" .Release.Namespace $secretName -}}
{{- if and $existing (hasKey $existing.data "hub_syncer_password") -}}
{{- $_ := set .Values "_resolvedHubSyncerPassword" (index $existing.data "hub_syncer_password" | b64dec) -}}
{{- else -}}
{{- $_ := set .Values "_resolvedHubSyncerPassword" (randAlphaNum 32) -}}
{{- end -}}
{{- index .Values "_resolvedHubSyncerPassword" -}}
{{- end -}}
{{- end -}}
