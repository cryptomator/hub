...
@NamedQuery(name = "Access.revokeUser", query = "DELETE FROM Access a WHERE a.id.vaultId = :vaultId AND a.id.deviceId IN (SELECT d.id FROM Device d WHERE d.user.id = :userId)")
public class Access {
